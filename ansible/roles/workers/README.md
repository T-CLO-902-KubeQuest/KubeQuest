...
## Overview
This Ansible role automates the process of adding worker nodes to an existing Kubernetes cluster. It is specifically designed to handle the joining of worker nodes to the cluster initialized by the control-plane.
...
2. **Dynamic Token Retrieval**: Instead of hardcoding tokens, the role uses `delegate_to` to run a command on the first node in the `masters` group. It generates a fresh join command using `kubeadm token create --print-join-command`.
3. **Cluster Join**: The worker node executes the dynamically retrieved command to join the cluster.
4. **Post-Join Verification**: After joining, the role checks that `kubelet.conf` was created to confirm the join succeeded.
...
- **Flexibility**: The role depends on Ansible groups (`masters` and `workers`), making it compatible with any environment (AWS, on-premise, etc.) as long as the inventory is correctly defined.
